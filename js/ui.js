...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Initial icon setup based on device
  const isMobile = window.matchMedia('(max-width: 768px)').matches;
  const icon = elements.togglePanelBtn.querySelector('i');
  if (isMobile) {
    icon.className = uiState.panelCollapsed ? 'fas fa-chevron-up' : 'fas fa-chevron-down';
  }
  
  // Add button-pressed style for animation
...
function handleOrientationChange() {
  ...
  const panel = elements.uiContainer;
  panel.style.transition = 'transform 0.3s ease';
  
  // Re-apply collapsed state with proper orientation-specific styling
  if (uiState.panelCollapsed) {
    const isMobile = window.matchMedia('(max-width: 768px)').matches;
    if (isMobile) {
      panel.style.transform = 'translateY(calc(100% - var(--mobile-title-height)))';
    } else {
      panel.style.transform = 'translateX(calc(var(--panel-width) - 50px))';
    }
  } else {
  ...
  if (isMobile) {
    ...
    icon.className = uiState.panelCollapsed ? 'fas fa-chevron-right' : 'fas fa-bars';
  }
}
...
/**
 * Simple debounce function to limit frequency of calls
 */
 ...
 * Enhanced for mobile support with proper icon changes
 */
function togglePanel() {
  ...
  // Update toggle button icon based on device type
  const icon = elements.togglePanelBtn.querySelector('i');
  
  if (isMobile) {
    // On mobile, use up/down chevron icons
    if (uiState.panelCollapsed) {
    ...
    // On desktop, use left/right icons
    if (uiState.panelCollapsed) {
  ...
  // Add touch animation for better feedback
  elements.togglePanelBtn.classList.add('button-pressed');
  setTimeout(() => {
    elements.togglePanelBtn.classList.remove('button-pressed');
  }, 200);
}

/**
 * Touch event handlers for drag panel on mobile
 * This enables the pull-down/pull-up gesture on mobile devices
 */
/**
 * Set up touch event handlers for mobile devices
 * Enhanced to properly handle panel dragging while preserving control interactions
 */
function setupTouchHandlers() {
  ...
  let wasDraggingSignificantly = false;
  let dragStartTime = 0;
  
  // Check if we're on a touch device
  if (!('ontouchstart' in window)) return;
  
  // Add touch handlers for all button-like elements
  setupTouchClickHandlers();
  
  // Handle panel dragging via the header
  header.addEventListener('touchstart', (e) => {
    startY = e.touches[0].clientY;
    currentY = startY;
    isDragging = true;
    dragStartTime = Date.now();
    wasDraggingSignificantly = false;
  ...
  document.addEventListener('touchmove', (e) => {
    ...
    // Track if this was a significant drag (to prevent click events afterwards)
    if (Math.abs(deltaY) > 10) {
      wasDraggingSignificantly = true;
    }
    
    // Only allow dragging in the appropriate direction based on current state
    if ((uiState.panelCollapsed && deltaY < 0) || (!uiState.panelCollapsed && deltaY > 0)) {
      const maxTravel = window.innerHeight * 0.3; // Limit how far it can be dragged
      const dragAmount = Math.min(Math.abs(deltaY), maxTravel);
      
      if (uiState.panelCollapsed) {
        // Dragging up when collapsed
        panel.style.transform = `translateY(calc(100% - var(--mobile-title-height) - ${dragAmount}px))`;
      } else {
        // Dragging down when expanded
        panel.style.transform = `translateY(${dragAmount}px)`;
      }
      
      // Prevent default scrolling when we're handling the drag
      e.preventDefault();
    }
  }, { passive: false });
  ...
  document.addEventListener('touchend', (e) => {
    ...
    isDragging = false;
    panel.style.transition = 'transform 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
    
    // Determine if the drag should toggle the panel state
    const deltaY = currentY - startY;
    
    // Toggle if significant drag OR quick flick
    const isQuickFlick = dragDuration < 300 && Math.abs(deltaY) > 20;
    if (Math.abs(deltaY) > 50 || isQuickFlick) {
      togglePanel();
    } else {
      // Reset to previous position
      panel.style.transform = uiState.panelCollapsed 
        ? 'translateY(calc(100% - var(--mobile-title-height)))' 
        : 'translateY(0)';
    }
  }, { passive: false });
}
...
/**
 * Set up touch tap/click handlers for all interactive controls
 * This ensures mobile devices properly handle button presses
 */
function setupTouchClickHandlers() {
  // iOS sometimes needs help with buttons not firing click events
  const allInteractiveElements = [
    ...
    ...Array.from(document.querySelectorAll('.toggle-container')),
    ...Array.from(document.querySelectorAll('.info-btn')),
    ...Array.from(document.querySelectorAll('.radio-container')),
    ...Array.from(document.querySelectorAll('.expand-btn')),
    ...Array.from(document.querySelectorAll('.section-header.expandable'))
  ];
  
  // Add touchend listeners to all interactive elements
  allInteractiveElements.forEach(element => {
    // Remove any existing listeners to prevent duplication
    element.removeEventListener('touchend', handleTouchEnd);
    
    // Add the touchend handler
    element.addEventListener('touchend', handleTouchEnd, { passive: false });
    
    // Add a visual active state for touch feedback
  ...
  // Add helper CSS for touch active state
  const style = document.createElement('style');
  style.textContent = `
    .touch-active {
      opacity: 0.8;
    ...
    .option-btn, .action-btn, .info-btn, .expand-btn {
...
/**
 * Handle touchend events on interactive elements
 * This ensures clicks work on iOS/touch devices
 */
function handleTouchEnd(e) {
  // Don't process if this was part of a drag or scroll
  ...
  // Only trigger if the finger is still on the original element
  if (this.contains(target) || this === target) {
    ...
    if (this.querySelector('input[type="radio"]') || this.querySelector('input[type="checkbox"]')) {
